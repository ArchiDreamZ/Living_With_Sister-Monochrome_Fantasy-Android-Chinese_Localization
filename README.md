...
截止至2024年5月，妹相随的最新版本为1.4.4（DLsite原作者版本号，也被称为1.44），对应Steam版本为1.04；HBOX的RPG Maker移植版最新版为1.1.8。
...
注意：彩绘是导致卡顿的主要原因，由于引擎底层缺陷无法解决只能缓解，因此我会对彩绘图片进行有损压缩，来尽可能同时拥有彩绘和流畅。卡顿问题的原因详见下文。
...
④**优化**：
原本我只是汉化，用别人的安装包作为基础，但是巴图版安装包体积很大，看似是用模板构建的，里面有很多残留的冗余无用的模板内容，并且还有无用且碍事的按键；英文版安装包有着屏幕适配问题，无法全屏游玩；滑稽酱版内部结构几乎与英文版完全一样。
...
## 汉化更新流程说明
该仓库还未完成精修，会频繁更新，在群中内测。会分阶段发布预览版进行测试，历次改动说明可查看：[Commits](https://github.com/ArchiDreamZ/Living_With_Sister-Monochrome_Fantasy-Android-Chinese_localization/commits/main/) ，每次新的汉化版本发布时，也会总结在发布介绍内。

我使用Translator++翻译软件进行翻译，第一步我会先导入RPG Maker引擎已有的翻译作为参照，例如Don群友翻译和巴图勇士翻译。这样在发布预览版时，即使我没有精翻完毕也有备选翻译。

第二步我会正式开始翻译，使用Wolf编辑器来查找神乐汉化与Don汉化的文本，将它们一条条粘贴进翻译器。

第三步我会进行润色，从各版本翻译中挑选最适合的翻译作为最终译本。

#### 当前进度
目前已经完成第一步，将“群友汉化”与“巴图汉化”导入到了Translator++用于参照与候补，现在正在一条条复制神乐汉化到Translator++中，并对比Don汉化来选出最终翻译。

正在群中进行内测，958472475（非资源群，资源在Github下载），欢迎参与测试反馈汉化问题。

## 下载
目前能使用的仅有针对巴图版的汉化修改版，从头翻译的大工程还在进行中。

是我在开始正式这个项目之前，先自己对巴图勇士的版本进行了一番修复，补全按键选项翻译，重翻少量剧情对话，并将GUI、状态栏、拟声词条等的图片替换为神乐的翻译版，以及更新了彩绘贴图。
...
②**导入PC版翻译**：PC的Wolf引擎的文本没法导入软件，会乱码。听说付费版的Translator++可以导出Wolf引擎的文本，不会乱码。

但目前我还没有付费版，因此我使用Wolf RPG Editor对神乐的官方汉化与Don的汉化进行查看，手动将里面的文本一条一条复制出来，粘贴在Translator++里。最后导出注入翻译。

③**按键失灵与丢失问题**：如果翻译不当，就会出现按键失灵，甚至部分按键直接消失这种恶性bug，所以巴图翻译与群友翻译都没有完全翻译按键，故意将那几个按键留着不翻译。

而我使用Translator++翻译也遇到了这个问题，最终我发现了原因。按键文本都是直接与条件判断代码写在一起的，会有多个检测按键的代码，改了一个后，其他的代码就识别不到这个按键了。因此要改就全改才行。而Translator++居然无法识别到所有按键文本，因此也出现了失灵与丢失现象。

最后我的解决方法就是，在Translator++导出文本后，再手动在Notepad++用宏批量替换按键文本。

④**贴图切分**：RPG Maker版和Wolf版的贴图有些不同：
Wolf版支持.jpg，但是RPG Maker版只支持.png格式；Wolf版使用的连图，好几张图片连在一起，而RPG Maker版需要将这些连着的图裁切开，切成单张540px。

连图都是多张540px的图片连成一起的，每个连图不一定是几张连一起。我没找到适合用PS的动作功能批处理不同连图的方法，于是我使用一些图片切分小软件，BigShear、七彩色图片批量处理工具，以及Advanced Renamer文件批量重命名工具来切分图片。
